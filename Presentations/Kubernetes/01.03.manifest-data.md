...
But where can i find information on how can i add data to the POD and other objects manifests?

One option is the official web page

https://kubernetes.io/docs/reference/generated/kubernetes-api/v1.19/#pod-v1-core

You can see the fields like metadata, apiversion, kind, spec that we have already seen in out yaml files.

![pods](images/pod-manifest-wp.JPG)

If you like more info for the spec session you can go further below

![pods](images/pod-manifest-wp2.JPG)


But it's also possible to find this information via command line.

You can use kubectl explain 'obecjt'.
In out case 'kubectl explain pod'
...
There are explanations on how to use the properties.

If you see a property of type Object. That means it's an object and there are other fields inside it.
So if you want more details you can use. 'kubectl exaplain.pod.(object)'

Lets say we want more information on spec.
...
Now let's even say you don't want to go item for item.
...
Im not going to paste the output here because it's too big. 
But you can save it on a file and use search for the fields you like.